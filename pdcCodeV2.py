...
# check if the file is .nc1 and if the length of the filename is greater than 25 characters
    if file_path.endswith(".nc1") and len(filename) >= 25:
        new_file_path = os.path.join(directory, filename[10:])
        with open(file_path, "r") as file_obj:
            lines = file_obj.readlines()
# Modify lines 4 and 5 based on their length and Ensure the file has at least 5 lines to avoid index errors
        if len(lines) >= 5: 
            for i in [3, 4]: 
